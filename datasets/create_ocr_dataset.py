...
def PDFA() -> pd.DataFrame | None:
    ...
    for i, pdf_name in enumerate(tqdm.tqdm(pdf_files, desc='Processing PDFA files')):
        ...
        images = pdf2image.convert_from_path(pdf_path, poppler_path=r"C:\poppler\Library\bin")
        for ind, (img, page) in enumerate(tqdm.tqdm(zip(images, data['pages']), desc=f'Processing {i} page')):
            for sec_ind, (text, box) in enumerate(zip(page['words']['text'], page['words']['bbox'])):
                box = [
...
def KOTHD() -> pd.DataFrame | None:
    ...
    for i, ann in enumerate(tqdm.tqdm(annotations, desc='Processing KOTHD')):
        try: 
            with open(os.path.join(annotations_path, ann), encoding='utf-8', mode='r') as j:
                data = json.load(j)
...
def WikiKZ() -> pd.DataFrame:
    ...
    for i, data in enumerate(tqdm.tqdm(datas, desc='WikiKZ processing')):
        ...
        with open(os.path.join(full_directory_data, data), 'r', encoding='utf-8') as txt:
            for j, line in enumerate(txt):
                try:
                    ...
                    box = [
                        (box_list[0] + w * random.uniform(-0.05, 0.05), box_list[1] + h * random.uniform(-0.05, 0.05)), 
                        (box_list[2] + w * random.uniform(-0.05, 0.05), box_list[1] + h * random.uniform(-0.05, 0.05)),
                        (box_list[2] + w * random.uniform(-0.05, 0.05), box_list[3] + h * random.uniform(-0.05, 0.05)),
                        (box_list[0] + w * random.uniform(-0.05, 0.05), box_list[3] + h * random.uniform(-0.05, 0.05))
                    ]
                    croped = crop_masked_rectangle(image, box)
