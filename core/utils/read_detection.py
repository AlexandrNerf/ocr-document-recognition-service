...
def read_detections(
    ...
    Args:
        images (List[np.ndarray] | List[Image.Image] | str | List[str]):
        Список изображений или путь к папке с ними
        txt_paths (List[str] | str):
        Путь до детекций (формат txt) или список детекций при labels_from_file=False
        box_format (str): Формат ('yolo' или 'pascal_voc')
        labels_from_file (bool): Если True, то txt_paths должен быть путь до папки
                        с txt файлами, иначе txt_paths должен быть список детекций
    """
    detections = []
    imgs = []
    ...
    for image, txt_path in zip(images, txt_paths):
        ...
        for line in lines:
            try:  # noqa: WPS229
                ...
                x_cn, y_cn, w_n, h_n = list(
                    map(float, coords_str.split(" "))
                )  # noqa: WPS221
...
def crop_masked_rectangle(  # noqa: WPS210, WPS234
    image: Union[np.ndarray, Image.Image, str],
    box: Union[List[tuple[int, int]], List[tuple[float, float]]],  # noqa: WPS221
    ...
    Args:
        ...
        Входное изображение в формате BGR или PIL Image,
        размер (H, W, 3), или путь к файлу.
        box (list[tuple[int, int]] | list[tuple[float, float]]):
        Координаты четырёхугольника [(x1, y1), (x2, y2), (x3, y3), (x4, y4)].
    Returns:
        out (np.ndarray | Image.Image): Вырезанное изображение в формате BGR,
                                        размер (h, w, 3),
    ...
    summary = bbox.sum(axis=1)  # x + y
    diff = np.diff(box, axis=1)  # x - y

    # Определяем углы
    top_left = bbox[np.argmin(summary)]
    bottom_right = bbox[np.argmax(summary)]
    ...
    width = int(
        max(
            ...
            np.linalg.norm(bbox[2] - bbox[3]),  # noqa: WPS221
        )
    )
    height = int(
        max(
            np.linalg.norm(bbox[0] - bbox[3]),
            np.linalg.norm(bbox[1] - bbox[2]),  # noqa: WPS221
        )
    )
    ...
    return warped if image is np.ndarray else Image.fromarray(warped)
