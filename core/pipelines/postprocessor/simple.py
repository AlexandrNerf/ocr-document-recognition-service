...
class SimplePostprocessor(Postprocessor):
    ...
    def rotate_polygon_boxes(self, polygons: np.array) -> list[tuple[int, np.array]]:
        ...
        def rotate_points_image_coords(
            points: np.array, angle: float, center: tuple[float, float] = (0, 0)
        ):
            """
            ...
            x_new = cos_angle * (x - cx) + sin_angle * (y - cy) + cx  # noqa: WPS221
            y_new = -sin_angle * (x - cx) + cos_angle * (y - cy) + cy  # noqa: WPS221
        ...
        straighten_polygons = [
            (i, rotate_points_image_coords(poly, mean_angle, doc_center))
    ...
    def sort_polygon_box_indices(
        ...
        straighten_polygons = sorted(
            straighten_polygons, key=lambda p: self.poly_center(p[1])[0]
        )
        ...
        while swapped:
            ...
            for i in range(length):
                if self.sort_two_boxes(
                    straighten_polygons[i], straighten_polygons[i + 1], threshold
                ):
                    straighten_polygons[i], straighten_polygons[i + 1] = (
                        straighten_polygons[i + 1],
                        straighten_polygons[i],
                    )
                    swapped = True
    ...
    def postprocessing(
        ...
        Returns:
            out (List[Detection]): Детекции с кропами
                    с правильным порядком вершин и самих боксов
        """
